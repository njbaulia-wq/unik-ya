...
> E2E 404 sempat flaky (3x diagnosis: presisi locator → double-main saat
> streaming → cold render ~3 dtk); fix final: timeout eksplisit 20 dtk.

## P0 — Security (perbaiki dulu)

- [x] **F1 — JSON-LD XSS breakout.** `src/app/products/[slug]/page.tsx:113`:
...
- [x] **F2 — CSRF di POST /api/contact-click.** Tanpa origin check; pihak
...
- [x] **F3 — `searchParamsSchema` strictObject menolak `utm_*`.**
...
- [x] **F7 — Admin tak bisa suspend creator (PRD §22).**
...
- [x] **F8 — Dashboard tanpa Settings (PRD §19).**
  Fix: `/dashboard/settings` (info akun + keluar), tambah link nav.
- [x] **F9 — Login abaikan `?next=`.** Middleware kirim `?next=` tapi
...
- [x] **F10 — Kategori tanpa canonical.** Fix: `generateMetadata`
  `/categories/[slug]` pakai canonical + deskripsi kategori.
- [x] **F11 — Gambar tanpa optimasi (PRD §41).** `ProductCard` + detail
